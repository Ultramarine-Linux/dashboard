...
const setUsernameParams = type({ userId: 'string', username: 'string' });
export const setUserUsername = command(setUsernameParams, async (params) => {
	const { db } = await requireCurrentAdmin();
	const target = await db.query.user.findFirst({ where: eq(user.id, params.userId) });
	if (!target) error(404, 'User not found');

	const trimmed = params.username.trim() || null;
	await db.update(user).set({ username: trimmed }).where(eq(user.id, params.userId));
	return { userId: params.userId, username: trimmed };
});
