...
export class AdminUsersState {
	...
	async setUserUsername(userId: string, username: string) {
		this.adminUserSaving = { ...this.adminUserSaving, [userId]: true };
		try {
			const result = await setUserUsername({ userId, username });
			this.adminUsers = this.adminUsers.map((u) =>
				u.id === userId ? { ...u, username: result.username } : u
			);
			if (this.selectedUser?.id === userId) {
				this.selectedUser = { ...this.selectedUser, username: result.username };
			}
			await invalidate('app:admin-users');
		} catch (err) {
			toast.error(getErrorMessage(err, 'Failed to update username'));
		} finally {
			this.adminUserSaving = { ...this.adminUserSaving, [userId]: false };
		}
	}
